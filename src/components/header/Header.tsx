...
export const Header = React.memo(withRouter(
  ({ location }: Partial<HeaderProps>) => {
    const tradeType = useRouteToTradeType();
    const label = tradeType ? capitalize(tradeType) : 'Market';
    
    const configs = useMemo(() => getHeaderConfig({ marketLbl: label, tradeType }), [tradeType, label]);
    
    const match = matchPath(location.pathname, { path: `/${tradeType === AppTradeType.DERIVATIVE ? 'derivative' : 'exchange'}/:symbol` });
    const symbol = _get(match, ['params', 'symbol']);

    const marketHeader = useMemo(() => [getHeaderMarketDropdown({label, tradeType})], [label, tradeType]);
