...
import { getBalances, getBalancesTotalAllWallets } from '@/selectors/balance.selectors';
...
import { FundingType, PositionSide, PositionType } from '@/constants/position-enums';
...
const Row = ({ title, data }) => <div className="d-flex d-align-items-center d-justify-content-space-between">
...
class BalanceContainer extends React.Component<any, BalanceContainerState> {
	...
	renderMargin() {
		const { markPrice, symbolEquity, executedLongPosition, executedLongCash, 
			executedShortCash, executedShortPosition, mmr, btcBalance,
		} = this.props;
		...
		let positions = [];
		let totalUnrealizedPnl = 0, equity = 0, maintenanceMargin = 0, availableMargin = 0, 
		...
		if (executedLongPosition !== undefined && markPrice !== undefined) {
			...
			totalUnrealizedPnl = positions.reduce((total, p) => +add(p.pnl, total), 0);
			maintenanceMargin = positions.reduce((total, p) => +add(p.margin, total), 0);
		...
		const c = totalUnrealizedPnl > 0 ? greenText() : totalUnrealizedPnl < 0 ? redText() : '';
		return <div className="balances__margin">
			<div className="balances__margin__info">
				<Row title="Balance" data={`${formatNumber({ number: btcBalance, decimals: 2 })} USDT`} />
				<Row title="Equity" data={`${formatNumber({ number: equity, decimals: 2 })} USDT`} />
				<Row title="Available Margin" data={`${formatNumber({ number: availableMargin, decimals: 2 })} USDT`} />
				<Row title="Maintenance Margin" data={`${formatNumber({ number: maintenanceMargin, decimals: 2 })} USDT`} />
				<Row title="Unrealised PNL" data={<span className={c}>{formatNumber({ number: totalUnrealizedPnl, decimals: 2 })} USDT</span>} />
			</div>
			<div className="equiv-wrap right-align">
				<div className="total-equivalent">
					<span>{formatNumber({ number: usedMargin, decimals: 2})}% Margin Used</span>
				</div>
	...
	render() {
		...
		return <div className="balances">
			<Collapsible
				title={<Tabs elements={tabConfigs} selected={selectedTab} onChange={this.onTabChange} />}
				toolbar={<BalanceEyeToggle />}
			>
				{selectedTab === 'asset' ? <Balances {...this.props} /> : this.renderMargin()}
			</Collapsible>
		</div>
	}
}
