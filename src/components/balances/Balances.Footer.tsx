...
class BalancesFooter extends React.PureComponent<Partial<BalancesFooterProps>> {
	...
	constructor(props) {
		...
		this.changeTotalEquivalentCurrency = this.changeTotalEquivalentCurrency.bind(this);
	}

	changeTotalEquivalentCurrency() {
		const {
			totalEquivalentCurrency,
			setSettingsTotalEquivalent,
		} = this.props;

		let nextCurrency = totalEquivalentCurrency === EQUIV_CCY ? 'BTC' : EQUIV_CCY;;
