...
export const LastTick = React.memo(({price, symbol, showNumber = true}: any) => {
  ...
  const classes = classNames('cpn-lastTick', diff > 0 ? greenText() : diff < 0 ? redText() : '', {
    'price-down': !(diff >= 0),
  });

  return (
    showNumber ? <NumberDiff classes={classes} number={price} decimals={decimals} suffix={icon}/> 
