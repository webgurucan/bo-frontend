...
export const Item = React.memo(({ symbol, price, amount, date, side }: TradeItemProps) => {
  const pClasses = `trade-history__item__price ${side === OrderBookSideEnum.BID ? greenText() : redText()}`;
  ...
  return (
    <div className="animsOn d-flex trade-history__item">
      ...
      <div className="trade-history__item__date"><span>{hhmmss(date)}</span></div>
    </div>
  );
});
...
export const TradeHeader = () => (
  <div className="d-flex d-align-items-center d-justify-content-space-between trade-history__header">
    <div className="trade-history__header__price"><span>Price</span></div>
    <div className="trade-history__header__amount"><span>Size</span></div>
    <div className="trade-history__header__date"><span>Time</span></div>
  </div>
)