...
import { toggleWorkspaceSetting, updateUISetting } from '@/actions/ui-setting.actions';
...
const mapDispatchToProps = dispatch => ({
  ...
  updateInterval(nextInterval) {
    dispatch(updateUISetting({ key: 'chart_interval', value: nextInterval, persist: false }))
  },
  updateChartTheme(theme) {
    dispatch(updateUISetting({ key: 'current_chart_theme', value: theme, persist: false }))
  },
  closePosition(amount, symbol, side) {
    // dispatch(closePosition({side, symbol, amount}))
  },
  closeCard(e) {
    dispatch(toggleWorkspaceSetting({key: WorkspaceSettingEnum.CHART, persist: false}))
  }
});
...
class InternalWrapper extends React.PureComponent<any> {
  ...
  constructor(props) {
    ...
    this.persistChartSettingsThrottled = _debounce(this.persistChartSettings, 3000, { maxWait: 10000 })
  }
  ...
  persistChartSettings(chartSettings?) {
    const chartSettingsToPersist = (chartSettings !== undefined) ? chartSettings : this.props.chartSettings;

    this.props.dispatch(updateUISetting({ key: getChartSettingsKey(), value: chartSettingsToPersist, persist: false }))
  ...
  updateChartSettings(chartSettings, persistImmediately) {
    ...
    if (persistImmediately) {
      ...
      this.props.dispatch(updateUISetting({ key: getChartSettingsKey(), value: chartSettings, persist: false }))
