...
import { MS_IN_INTERVAL, CANDLES_FETCH_LIMITS, CANDLES_FETCH_ATTEMPS } from "./Chart.constants";
...
export function generateEmptyCandle(time, price) {
	return ({
		time, open: price, close: price, high: price, low: price, volume: 0,
	});
}

export const getMsInInterval = interval => MS_IN_INTERVAL[interval];

export function extractCandlesFromSnapshot(candles: number[], interval: string, endTime: number) {
	if (!candles) {
		return null;
	}
...
function determineFetchLimit(start, end, interval) {
	const estimatedNumberOfCandles = Math.ceil((end - start) / getMsInInterval(interval));

	const maxLimit = CANDLES_FETCH_LIMITS[CANDLES_FETCH_LIMITS.length - 1];

	return CANDLES_FETCH_LIMITS.find(item => (item > estimatedNumberOfCandles || item === maxLimit));
}

const historyURL = (resolution, symbol) => `https://api.binance.com/api/v3/klines?${generateSymbolKey(resolution, symbol)}`;

function sleep(ms) {
	return new Promise(resolve => setTimeout(resolve, ms))
}

export async function fetchCandlesAsync(start, end, interval, pair, onSuccess, onError) {
	...
	fromFetch(url, {
		...
		retryWhen(errors => errors.pipe(
			delay(1000),
			take(CANDLES_FETCH_ATTEMPS),
		)),
		timeout(3000),
	).subscribe(data => {
		//@ts-ignore
		let candles = (data || []).sort((a, b) => a[0] - b[0])
			.map(item => ({
				time: item[0],
				open: +item[1],
				high: +item[2],
