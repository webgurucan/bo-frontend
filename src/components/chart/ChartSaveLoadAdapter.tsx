...
class ChartSaveLoadAdapter extends React.PureComponent<any> {
	...
	componentDidUpdate(prevProps) {
		...
		if (chartsList !== prevProps.chartsList) {
			...
			callCallbacksIgnoreExceptions(this.listChartsPromisesMap, convertedCharts);
	...
	removeChart(chartId) {
		...
		const updatedChartsList = chartsList.filter(chart => chart.id !== chartIdString);
		setChartsList(dispatch, updatedChartsList);
...
export default connect(null, null, null, { forwardRef: true })(ChartSaveLoadAdapter);