...
const formatAlertPrice = number => formatNumber({ number, significantFigures: 5 });

export function createOrderPrimitive(chart, order, changeOrderPrice, modifyOrder, cancelOrder, orders) {
	const style = getCurrentStyle();

	const orderType = order.side === 'SELL' ? 'ask' : 'bid';
	const orderText = orderTypeLetter(order);

	return chart.createOrderLine({ disableUndo: true })
		// .onMove(function() {
		...
		.setQuantityBackgroundColor(style.ordersVis[orderType].quantityBackgroundColor)
		.setCancelButtonIconColor(style.ordersVis[orderType].cancelButtonIconColor)
		.setCancelButtonBorderColor(style.ordersVis[orderType].cancelButtonBorderColor)
		.setCancelButtonBackgroundColor(style.ordersVis[orderType].cancelButtonBackgroundColor)
...
export function createPositionPrimitive(chart, position, closePosition) {
	...
	return chart.createPositionLine({ disableUndo: true })
		.onClose(function () {
			//@ts-ignore
		...
		.setText(`P/L: ${formatPosition(position.pnl)} (${formatPosition(position.pnlPerc)}%)`)
		.setLineLength(80)
...
export function createPriceAlertPrimitive(chart, alertPriceStr, alertPair, createAlert, removeAlert) {
	const style = getCurrentStyle();
