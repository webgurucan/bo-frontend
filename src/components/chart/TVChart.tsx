...
import { CHART_ROOT_ELEMENT, RESOLUTION_MAP, SERVER_SIDE_CUSTOMIZATION } from './Chart.config';
import {
	updateMarks,
	sanitizeHtml,
	isMarksMessage
...
import { ADD_EMPTY_CANDLE_TO_END_DELAY, SNAPSHOT_CANDLES_NUMBER, DOUBLE_CLICK_THRESHOLD, INTERVAL, HISTORY_LOOKBACK_CANDLES, ANY_SYMBOL_MARK } from './Chart.constants';
...
export default class TVChart extends PureComponent<Partial<TVChartProps>> {
	...
	constructor(props) {
		...
		this.onResetButtonClick = _throttle(this.onResetButtonClick.bind(this), 3000);
	...
	componentDidMount() {
		...
		this._subscription$ = chartSubscriber().subscribe((data: any) => {
			if(this.chartReady && data.pair === this.props.pair && data.interval === RESOLUTION_MAP[this.props.interval]) {
	...
	initializeChartIfNecessary() {
		...
		const { pair, lastPrice, chartSettings, timeDifference, interval } = this.props;
	...
	componentDidUpdate(prevProps) {
		...
		const {
			pair, interval, isSubscribed, candles, priceAlerts, orders, position,
		} = this.props;
	...
	render() {
		...
		return (
			<div className="h-100">
				...
				<div id={CHART_ROOT_ELEMENT} style={{width: '100%', height: '100%'}} />
			</div>
		);
	}
	...
	onChartReady() {
		...
		this.getChart().crossHairMoved(({ price }) => { this.chartCrosshairLastPrice = price });
		this.updatePriceAlerts();
		this.updateOrders();
		this.updatePosition();
	}
	...
	updatePriceAlerts() {
		...
		this.updateTradingPrimitives(
			'priceAlerts', 'priceAlertsPrimitives', items => items, createPriceAlertPrimitive, pair, addPriceAlert, removePriceAlert
		);
	}

	updateOrders() {
		const {
			pair, changeOrderPrice = NOOP, modifyOrder = NOOP, cancelOrder = NOOP,
			orders
		} = this.props;
		// console.warn('[chart] updateOrders >>> orders', orders);

		const mapper = orders => _filter(Object.values(orders), (order: any) => order.pair === pair);

		this.updateTradingPrimitives(
			'orders', 'ordersPrimitives', mapper, createOrderPrimitive, changeOrderPrice,
			modifyOrder, cancelOrder, orders
		);
	}
	...
	updatePosition() {
		...
		this.updateTradingPrimitives('position', 'positionPrimitives', position => [position], createPositionPrimitive, closePosition)
		// this.updateTradingPrimitives('position', 'liquidationPrimitives', position => [position], createLiquidationPricePrimitive)
	}

	updateTradingPrimitives(propName, primitivesName, mapToArrayFunction, createFunction, ...createFuncArgs) {
		...
		if (propsItem) {
			this[primitivesName] = mapToArrayFunction(propsItem).map(item => createFunction(chart, item, ...createFuncArgs))
		}
	}
	...
	addEmptyCandleToEnd() {
		...
		if (candles && candles.length > 0) {
			...
			if (timeNow > (nextCandleTime + ADD_EMPTY_CANDLE_TO_END_DELAY)) {
				...
				const emptyCandle = generateEmptyCandle(nextCandleTime, lastCandle.close);
	...
	resetChart() {
		...
		try { this.widget.remove() } catch (e) { }
		this.widget = null;
		this.priceAlertsPrimitives = null;
		this.ordersPrimitives = null;
		this.positionPrimitives = null;
	...
	resetChartData() {
		if (!this.chartReady || !this.widget || !this.widget.activeChart())
			return;
	...
	requestCandlesFetch() {
		...
		fetchCandlesAsync(startTime, end, interval, pair, this.onFetchCandlesSuccess, this.onErrorCallback)
	}
	...
	onFetchCandlesSuccess(historicalCandles, hasMoreData) {
		...
		if (candles.length > 0 && candlesToAdd.length > 0 && (candlesToAdd[candlesToAdd.length - 1].time === candles[0].time)) {
			candlesToAdd.pop();
		}
	...
	subscribeBars(symbolInfo, resolution, onRealtimeCallback, listenerGuid, onResetCacheNeededCallback) {
		...
		const timer = resolution.includes('D') ? MAX_INTERVAL : (+resolution * MULTIPLY_RESOLUTION * 60 * 1000);
	...
	getBars(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback, firstDataRequest) {
		...
		} else if (((rangeStartDate * 1000) < getEarliestCandleTime(this.candles)) && !this.noMoreHistoricalCandles) {
			this.requestCandlesFetch();
		} else {
			this.fulfillGetBarsRequest();
		}
	...
	getMarks(symbolInfo, from, to, onDataCallback) {
		...
		Object.keys(rawChartMarks).forEach((id) => {
			...
			const {
				ts, color_bg, color_border, color_text, label, size_min
			} = mark;

			const mSymbol = mark.symbol;
			const symbolMatch = !mSymbol || mSymbol === ANY_SYMBOL_MARK || mSymbol === symbol;
