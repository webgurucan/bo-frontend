...
import { WebSocketChannelEnum, WebSocketKindStateEnum } from "@/constants/websocket.enums";
...
const Subscriber = React.memo(({ channel, subscribeFunc, unsubscribeFunc, dependencies = {}, isSocketReady }: Partial<SubscriberProps>) => {
  ...
  return shallowCompareObjects(prevProp.dependencies, nextProps.dependencies) && prevProp.isSocketReady === nextProps.isSocketReady
});
...
const mapStateToProps = (state, props: SubscriberProps) => {
  ...
  return {
    isSocketReady: SingletonWSManager.isMarketWsById(wsId) ? socketState === WebSocketKindStateEnum.OPENED : socketState === WebSocketKindStateEnum.AUTHORIZED
  }
}
