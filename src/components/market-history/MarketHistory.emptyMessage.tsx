...
const MarketHistoryEmptyMessage = ({ isLoggedIn }: Partial<MarketHistoryEmptyMessageProps>) => {
  if (isLoggedIn)
    return null;

  return <div className="market-history__empty_message">
    <p>
      <Link to={RoutePaths.LOGIN}>Log in</Link> or <Link to={RoutePaths.REGISTER}>Register Now</Link> to trade
  </p>
  </div>
}
