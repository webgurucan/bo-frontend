...
function generateFooterCells(columns, totalPositions, totalUnealisedValue, totalRealisedValue) {
  const marginRight = 5;

  const generateDiv = (index, style, content) => <div key={`pos-${index}`} style={style}>{content}</div>;
...
const T = () => <div className="text-right font-size-12 font-medium"><div>$12,187.41 USD</div></div>
