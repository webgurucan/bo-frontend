...
const getPositionColumns = function () {
  return [{
    ...
    renderCell: function ({ cellData }) {
      ...
      return <Tooltip tooltipContent={symbol}><span>{symbol}</span></Tooltip>
    }
  }, {
    dataKey: 'size',
    //@TODO i18n
    ...
    renderCell: function ({ cellData }) {
      return (
        <div className="cpn-virtualized-table__cellwrapper text-right font-semi-bold">
          <span><NumberFormat number={cellData} decimals={2} /></span>
        </div>
      );
    }
  }, {
    ...
    renderCell: function renderCell({ cellData, rowData }) {
      ...
      return <div className="cpn-virtualized-table__cellwrapper text-right font-semi-bold">
    ...
    renderCell: function renderCell({ cellData, rowData }) {
      const c = greenText();
      return <div className="cpn-virtualized-table__cellwrapper text-right font-semi-bold">
        <span className={c}><NumberFormat number={cellData} decimals={3} />(<NumberFormat number={rowData.pnlPerc} decimals={2} />%)</span>
      </div>
    }
  }, {
    ...
    renderCell: function ({ cellData }) {
      ...
      const tpsl = (takeProfit && stopLoss) ?
        (<span className="font-semi-bold">
          <span className={greenText()}><NumberFormat number={takeProfit} decimals={2} /></span>/
          <span className={redText()}><NumberFormat number={stopLoss} decimals={2} /></span>
        </span>)
    ...
    renderCell: function ({ rowData }) {
      ...
      return (
        <div className="cpn-virtualized-table__cellwrapper text-right font-semi-bold">
          <ClosePositionBtn position={rowData} closePosition={(price, qty) => {
