...
class PositionCloseModal extends React.Component<Partial<PositionCloseLimitModalProps>, PositionCloseLimitModalState> {
  constructor(props) {
    super(props);
  ...
  onConfirmBtnClick(e) {
    ...
    const { confirmFunc, confirmDisablePopup, isMarket, closePopup } = this.props;

    confirmFunc(+price, +qty);

    if (disablePopup) {
      const key = isMarket ? 'enabled_position_close_market_popup' : 'enabled_position_close_limit_popup';
      confirmDisablePopup(key);
    }
    const mid = isMarket ? 'position_market_close_popup' : 'position_limit_close_popup';
    closePopup(mid);
  }
  ...
  renderBody({ renderButtons }): ReactNode {
    ...
    return (
      <div className="edit-order__body position__edit__body__ctn">
        {!isMarket && <div className="position__edit__body__row">
          <GroupInput
            addonAfter={quote}
        ...
        <div className="position__edit__body__row">
          <OrderFormQuantityButtons balance={balance} side={side} onClick={this.onPercQuantityBtnClick} />
        </div>
        ...
        <div className="d-flex font-size-11 d-justify-content-flex-end font-semi-bold d-align-items-center">
          <InputCheckboxInline value='1' checked={disablePopup} onChange={this.onCheckboxChange} label="Don't show again" />
        </div>
      </div>
    )
  }
