...
class OrderFormAdvanced extends React.PureComponent<OrderFormAdvancedProps> {
  render() {
    const { pair, typeId, displaySize, onDisplaySizeChange, refreshSize, onRefreshSizeChange } = this.props;
    ...
    const priceRegex = decimalPlacePrice ? 
      ...
      : decimalPlacePrice === null ? floatingPointRegex : numberRegex;

    return (
      shouldDisplayAdvancedGroups(typeId) ? <OrderFormCollapseArea
        title="Advanced"
      >
        <div className="order-form__input-wraper--1-1">
          <OrderFormInputWithInfo pattern={priceRegex} placeholder="Display Size" value={displaySize || ''} onChange={onDisplaySizeChange} tooltipContent="Value entered will be shown on Order Book rather than the size of your entire order." />
          <OrderFormInputWithInfo pattern={priceRegex} placeholder="Refresh Size" value={refreshSize || ''} onChange={onRefreshSizeChange} tooltipContent="???" />
        </div>
      </OrderFormCollapseArea> : null
    );
  }
}
