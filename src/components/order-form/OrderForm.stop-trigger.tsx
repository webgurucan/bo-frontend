...
class OrderFormStopTrigger extends React.PureComponent<OrderFormStopTriggerProps> {
  ...
  render() {
    const { enabledStopTrigger, onToggleStopTrigger, selectedCloseTrigger, onCloseTriggerOptionChange } = this.props;

    return (
      <div className="order-form__close-trigger-ctn">
        <InputCheckboxInline checked={enabledStopTrigger} onChange={onToggleStopTrigger} label="Close on Trigger" />
        <Tabs disabled={!enabledStopTrigger} elements={this.state.tabs} selected={`${selectedCloseTrigger}`} onChange={onCloseTriggerOptionChange} tabType={TabTypes.RADIO_BUTTONS} containerClassName="border-radius" />
      </div>
    );
  }
}
