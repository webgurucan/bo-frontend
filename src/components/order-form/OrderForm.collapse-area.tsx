...
class OrderFormCollapseArea extends React.Component<OrderFormCollapseProps, OrderFormCollapseState> {
  state = {
    expanded: false
  }
  ...
  render() {
    ...
    return (
      <div className={cntClasses}>
        <div className="order-form__advanced-btn__title_ctn clickable" onClick={this.onToggleBtnClick}>
          <span className="order-form__advanced-btn__title">{btnTitle}</span>
        </div>
        {expanded && <div className="order-form__advanced-btn__body">
          {this.props.children}
        </div>}
      </div>
    );
