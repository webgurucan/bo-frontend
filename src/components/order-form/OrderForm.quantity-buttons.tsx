...
const QuantityButtons = ({ side, balance, options, onClick }: Partial<Props>) => {
  const className = 'order-form__quantity__ctn';

  return (
    <div className={className}>
      {options.map(({ label, value }) => <Tooltip key={label} tooltipContent={`Use ${label} balance`}>
        <span className="order-form-tag" onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
      ...
      <span>
        <InputTextInline max={100} onChange={(value) => {
          onClick(balance, +value / 100, side);
        }} useHandlers={false} type="number"/>
      </span>
    </div>
  );
};
