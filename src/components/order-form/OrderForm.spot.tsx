...
export class OrderFormSpot extends React.Component<Partial<OrderFormProps>, any> {
  ...
  private renderTab() {
    return <Tabs elements={this._spotTabConfig} selected={this.state.selectedTab} onChange={this.onTabChanged} tabType={TabTypes.RADIO_BUTTONS} containerClassName="border-radius" />
  }
  ...
  render() {
    ...
    return (
      <div className="order-form__wrapper">
        ...
        <OrderFormInputs balance={balanceAmount} side={+selectedTab} {...this.props} />
        <div className="btn-order__wrapper mb-10">
          <OrderSubmitButton className={btnClass} side={+selectedTab} onBtnClickFallback={this.sendOrder} label={label} />
        </div>
        <div className="d-flex font-size-11 d-justify-content-space-between font-semi-bold">
          <div className="d-flex d-flex-direction-column">
            <div>{isBuyOrder ? 'Buy' : 'Sell'} {base}</div>
            <div className="number-text">{formatNumber({ number: amount, decimals: getAmountDecimals(pair) })} {base}</div>
          </div>
          <div className="d-flex d-flex-direction-column">
            <div className="text-right">Available Balance</div>
            <div className="number-text text-right">{formatNumber({ number: balanceAmount, decimals: getAmountDecimals(pair) })} {balanceCCy}</div>
          </div>
        </div>
      </div>
    );
