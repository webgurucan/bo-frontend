...
import { getAmountDecimals, getPriceDecimals, getSymbols } from '@/exports/ticker.utils';
...
export class OrderFormDerivative extends React.Component<Partial<OrderFormProps>, OrderFormDerivativeState> {
  ...
  onLeverageInputChange(e) {
    ...
    if (e.target.value > maxLeverage || e.target.value < 1)
      return;
  ...
  private renderTab() {
    return <Tabs elements={this.state.tabs} selected={this.state.selectedTab} onChange={this.onTabChanged} tabType={TabTypes.RADIO_BUTTONS} containerClassName="border-radius" />
  }

  private sendOrder(clientOrderId: number, side: OrderSide, cb) {
    const { onOrderBtnClick } = this.props;

    onOrderBtnClick(clientOrderId, {side, selectedLeverage: this.state.leverage, marginType: this.state.selectedTab as MarginType}, cb);
  }

  private renderForm() {
    const { total, balances, wallet, pair, typeId, onRefreshSizeChange, onDisplaySizeChange, displaySize, refreshSize } = this.props;
    const [, quote] = getSymbols(pair);
    const balanceCCy = quote;

    const balanceAmount = _get(balances, [balanceCCy, walletNameFromId(wallet), 'available'], 0);
    
    return <>
      <OrderFormInputs balance={balanceAmount} {...this.props} />
      <div className="btn-order__wrapper derivative-order-btns mb-10">
        <OrderSubmitButton className="btn text-left buy" side={OrderSide.BUY} onBtnClickFallback={this.sendOrder} label="BUY / LONG" />
        <OrderSubmitButton className="btn text-right sell" side={OrderSide.SELL} onBtnClickFallback={this.sendOrder} label="SELL / SHORT" />
        <div className="spread pos-abs-center font-size-11">
          <span>0.55</span>
        </div>
      </div>
      <div className="d-flex font-size-11 d-justify-content-space-between font-semi-bold">
        <div className="d-flex d-flex-direction-column">
          <div>Order Value</div>
          <div className="number-text">{formatNumber({ number: total, decimals: getAmountDecimals(pair) })} USDT</div>
        </div>
        <div className="d-flex d-flex-direction-column">
          <div className="text-right">Available Margin</div>
          <div className="number-text text-right">{formatNumber({ number: 0, decimals: getAmountDecimals(pair) })} USDT</div>
        </div>
      </div>
      <OrderFormAdvanced
        pair={pair} 
    ...
    return (
      <div className="adjust-leverage">
        ...
        <div className="input-inline">
          ...
          <div className="input">
            <input type="text" pattern={numberRegex} value={leverage} onChange={this.onLeverageInputChange} />
            <span>x</span>
          </div>
        ...
        <div className="slider__ctn">
          <InputRangeSlider value={leverage} min={1} max={maxLeverage} onChange={this.onSliderChange} />
        </div>
        <div className="btn-list">
          <Button classes="btn btn-cancel" onClick={this.onCancelBtnClick}><span className="text--white">Cancel</span></Button>
          <Button classes="btn primary" onClick={this.onConfirmBtnClick}><span className="text--white">Confirm</span></Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="order-form__wrapper">
        ...
        <div className="order-form__body__ctn">
          {this.renderForm()}
          {selectedTab === 'isolate' && this.state.showSlider && this.renderLeverage()}
        </div>
      </div>
    );
  }
