...
import { calculateLP, getLabelOrderSide, getLabelOrderType, getLabelTIF } from '@/exports/order.utils';
...
class OrderFormConfirmOverlay extends React.Component<OrderFormConfirmOverlayProps, any> {
  state = {
  ...
  render() {
    const { orderType, side, price, amount, total, symbol, marginType, leverage, wallet, tif, mmr, closeOverlay, label } = this.props;
    const { disableOverlay } = this.state;
    ...
    const sideCcy = wallet === WalletType.DERIVATIVE ? quote : isBuy(side) ? base : quote;
    const marginTypeText = marginType === MarginType.CROSS ? 'Cross' : `Isolate ${leverage}x`;

    const colorClassName = isBuy(side) ? greenText() : redText();
    return (
      <div className="order-form__confirm-overlay">
        <h5>
          <span className={colorClassName}>{getLabelOrderType(orderType)} {getLabelOrderSide(side)} <span className="font-bold"><NumberFormat number={amount} decimals={decimalAmount} /></span></span>
          <span className="font-bold"> {sideCcy}</span> at a price of <span className="font-bold">{isMarketOrder(orderType) ? 'Market Price' : <NumberFormat number={price} decimals={decimalPrice} />}</span></h5>
        <div className="d-flex d-align-items-center d-justify-content-space-between mb-10">
          ...
          <span className="font-semi-bold">{total} {sideCcy}</span>
        </div>
        <div className="d-flex d-align-items-center d-justify-content-space-between mb-10">
        ...
        <div className="d-flex d-align-items-center d-justify-content-space-between mb-10">
          ...
          <span className="font-semi-bold">{
            formatNumber({ number: calculateLP({ leverage, price, amount, mmr, marginType, side }), decimals: decimalPrice })
          } {sideCcy}</span>
        </div>
        <div className="d-flex d-align-items-center d-justify-content-space-between mb-10">
        ...
        <div className={`btn-order__wrapper derivative-order-btns mb-10 ${!isBuy(side) ? 'btn_order_btns--sell' : ''}`}>
          <div className="order-form__button-container">
            <Button classes="btn btn-cancel gray" onClick={closeOverlay}>Cancel</Button>
          </div>
          <OrderSubmitButton className={`btn ${getLabelOrderSide(side)}`.toLocaleLowerCase()} side={side} onBtnClickFallback={this.onConfirmBtnClick} label={label} />
        </div>
        <div className="d-flex font-size-11 d-justify-content-flex-end font-semi-bold d-align-items-center">
          <InputCheckboxInline value='1' checked={disableOverlay} onChange={this.onCheckboxChange} label="Don't show again" />
        </div>
      </div>
    );
  }
