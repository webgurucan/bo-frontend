...
const TableColGroup = ({ headers }) => <colgroup>
  {headers.map(header => {
...
export const OrderBookSide = ({
  ...
  const headers = useMemo(() => getHeaders({ width, dualColumn, isBid, enabled1Click }), [width, dualColumn, isBid, enabled1Click]);

  const tableClasses = classNames('table-container', 'table', 'orderBookTable', 'no-hacks', 'table-stripped', {
    'asks': !isBid,
    'bids': isBid
  });
