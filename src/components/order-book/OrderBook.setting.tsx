...
import { toggleBooleanSetting, updateUISetting } from '@/actions/ui-setting.actions';
...
import { FixedDropdown, InputTextInline, Menu, MenuItem, RadioButton, RadioGroup, Switch } from '@/ui-components';
...
const BookSetting = ({
  ...
  const selectedColumnView = dualColumn ? OrderBookStyleEnum.DUAL_COL : OrderBookStyleEnum.SINGLE_COL;
  ...
  return (
    ...
    >
      <Menu>
        <RadioGroup className="select-column" selectedValue={selectedColumnView} onChange={toggleBookMode}>
        ...
        <MenuItem spaceBottom={true} content={
          <div className="orderbook__level-group">
            <div className="orderbook__input-group">
            ...
            <div className="input-desc">
              Group orders by price intervals.
            </div>
          </div>
        } />
        <MenuItem content={
        ...
        <MenuItem content={
          <div className="d-flex d-justify-content-space-between w-100">
            ...
            <Switch isActive={scrollable} onChange={() => { toggleScrollable() }} />
          </div>
        } />
        <MenuItem content={
          <div className="d-flex d-justify-content-space-between w-100">
            <div className="label">Depth Visualization</div>
            <Switch isActive={showDepth} onChange={() => { toggleDepth() }} />
          </div>
        } />
      </Menu>
    </FixedDropdown>
...
const mapStateToProps = (state) => ({
  ...
  enable1ClickConfirmPopup: getSetting(state)('show_orderbook_1_click_confirm_popup'),
});

const mapDispatchToProps = (dispatch) => ({
  toggleBookMode: function (mode: number, persist?: boolean) {
    const isDual = mode === OrderBookStyleEnum.DUAL_COL;
    dispatch(updateUISetting({ key: 'orderbook_dual_column', value: isDual, persist: true }))
  },
  toggleScrollable: function (persist?: boolean) {
    dispatch(toggleBooleanSetting({ key: 'orderbook_scrollable', persist: false }))
  },
  toggleDepth: function (persist?: boolean) {
    dispatch(toggleBooleanSetting({ key: 'orderbook_show_depth', persist: false }))
  },
  toggleEnabling1Click: function (showPopup: boolean) {
    if(showPopup) {
      dispatch(showModal('orderbook_confirm_1_click_popup', OrderBookConfirm1ClickPopup, { }));
    } else {
      dispatch(toggleBooleanSetting({ key: 'orderbook_1_click', persist: true }))
    }
  },
  changeLevel: function (level = 0, persist?: boolean) {
    dispatch(updateUISetting({ key: 'orderbook_zoom_level', value: level, persist: false }))
  }
});

export const BookSettingDropdown = connect(mapStateToProps, mapDispatchToProps)(BookSetting)