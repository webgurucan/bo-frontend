...
const getOrderColumns = function ({
  ...
  const baseColumns = [{
    ...
    renderCell: function ({ rowData }) {
      ...
      return <IconButton id="circle" cssmodule="fas" classes={`${c} font-size-9`} />;
    }
  }, {
    dataKey: 'symbol',
    //@TODO i18n
      ...
      return (
        <div className="cpn-virtualized-table__cellwrapper text-right font-semi-bold">
          <DisplayConfirmModalBtn
            ...
            popupData={props}
          >
            <NumberFormat classes={greenText()} number={cellData} decimals={8} />
          </DisplayConfirmModalBtn>
        </div>
      );
    }
    ...
    renderCell: function ({ rowData }) {
      ...
      return (
        <div className="cpn-virtualized-table__cellwrapper text-center font-semi-bold">
          <CancelOrderBtn tooltip="cancel this order" symbol={symbol} order={rowData} onClick={cancelOrder} />
        </div>
      );
    },
    renderHeader: function renderHeader() {
      ...
      const message = `Are you sure you want to cancel all${isStopOrder ? ' stop': ''} order?`;
      const tooltip = `Cancel all${isStopOrder ? ' stop': ''} order`;
      
      return <CancelAllOrderBtn
        tooltip={tooltip}
        message={message}
  ...
  if (orderKind === OrderKind.OPEN) {
    return insertsAt(6, [{
      ...
      renderCell: function ({ rowData }) {
        return (
          <div
            className="cpn-virtualized-table__cellwrapper text-right font-semi-bold"
          >
            <NumberFormat number={rowData.qty - rowData.execShares} decimals={8} />
          </div>
        )
      }
    }, {
      ...
      renderCell: function ({ rowData }) {
        ...
        const tpsl = (rowData.takeProfitPrice && rowData.stopLossPrice) ?
          (<span className="font-semi-bold">
            <span className={greenText()}><NumberFormat number={rowData.takeProfitPrice} decimals={2} /></span>/
            <span className={redText()}><NumberFormat number={rowData.stopLossPrice} decimals={2} /></span>
          </span>)
          : (<span className="font-semi-bold"><Icon cssmodule="far" id="plus"/> TP/SL</span>)
        return (
          <div
            className="cpn-virtualized-table__cellwrapper text-right font-semi-bold"
          >
        ...
        return (
          <div className="cpn-virtualized-table__cellwrapper text-right font-semi-bold">
            <DisplayConfirmModalBtn
              ...
              popupData={props}
            >
              <span>&#8805;</span> <NumberFormat number={cellData} decimals={2} />
            </DisplayConfirmModalBtn>
          </div>
        );
      }
