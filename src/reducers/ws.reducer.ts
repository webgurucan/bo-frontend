import { WS_OPEN, WS_DISCONNECTED, WS_AUTH, WS_UNAUTH, WS_CONNECT } from "@/actions/ws.actions";
...
export const wsReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    case WS_CONNECT: {
      ...
      return {
        ...state,
        wsCollection: _set(state.wsCollection, [id], WebSocketKindStateEnum.CONNECTING)
      };
    }
    case WS_OPEN: {
      const { id } = action;
      ...
      return {
        ...state,
        wsCollection: _set(state.wsCollection, [id], WebSocketKindStateEnum.OPENED)
      };
    }
    case WS_DISCONNECTED: {
      ...
      return {
        ...state,
        wsCollection: _set(state.wsCollection, [id], WebSocketKindStateEnum.IDLE)
      }
    }
    case WS_AUTH: {
      ...
      return {
        ...state,
        wsCollection: _set(state.wsCollection, [id], WebSocketKindStateEnum.AUTHORIZED)
      }
    }
    case WS_UNAUTH: {
      const { id } = action;
