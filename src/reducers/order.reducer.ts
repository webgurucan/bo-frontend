...
export const orderReducer = (state = initialState, action) => {
  switch (action.type) {
    case ORDER_NEW_ACCEPTED: {
      ...
      const str = '<orderType> <orderSide> of <amount> contracts of <symbol> at <price>.';
      ...
      return {
        ...
        updatedOrder: {
          ...
          message: strTemplate(str, {
            ...
            amount: formatNumber({ number: newOrder.qty, decimals: amountDecimals }),
            orderId: newOrder.orderId,
            symbol: 'BTC',
            price: formatNumber({ number: newOrder.price, decimals: priceDecimals })
          })
        }
      };
    }
    case ORDER_UPDATED: {
      const { msgType, newOrder } = action.payload as { msgType: MessageType, newOrder: OrderItem };
      ...
      switch (msgType) {
        ...
        case MessageType.EXECUTION: {
          ...
          const message = `${msgType === MessageType.CANCELLED ? 'Cancelled' : 'Filled'} <orderType> <orderSide> of <amount> contracts of <symbol> at <price>.`;
          const qty = msgType === MessageType.CANCELLED ? newOrder.qty : newOrder.execShares;
        ...
        case MessageType.ORDER_NEW: {
          ...
          const orders = _set({ ...state.orders }, [newOrder.clientOrderId], newOrder);
