...
class Container {
  ...
  public instances: Map<string | Symbol, ConcreteTarget> = new Map<
    string,
    ConcreteTarget
  >();
  ...
  alias(abstract: string, alias: string) {
    invariant(
      this.bound(abstract),
      `Cannot assign alias for abstract "${abstract}". Abstract has no binding.`
    );
  ...
  getDependencies(concrete: Function): any[] {
    ...
    for (let i = 0; i < depLength; i++) {
      ...
      invariant(
        resolved,
        `Unable to resolve "${elem.toString()}" dependency for "${concrete.toString()}"`
      );
  ...
  getBinding(abstract: string | Symbol): Binding | undefined {
    invariant(
      this.bindings.has(abstract),
      `No binding found for abstract "${abstract.toString()}"`
    );
  ...
  _bind(
    abstract: string | Symbol,
    concrete: Function | null = null,
    isShared: boolean = false,
    isConcreteCallback: boolean = false
  ) {
