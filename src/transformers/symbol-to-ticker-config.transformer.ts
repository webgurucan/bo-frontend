...
import { DailyTickerChangeModel, SymbolConfig } from "@/models/symbol-config.model";
...
export function symbolToTickerConfig(sc: SymbolConfig): TickerConfigModel {
  const { symbol, baseAsset, baseAssetPrecision, quoteAsset, quotePrecision, filters = [] } = sc;
  let priceFilter = filters.find(({filterType}) => filterType === 'PRICE_FILTER') || {};
  let qtyFilter = filters.find(({filterType}) => filterType === 'LOT_SIZE') || {};
