...
import { DisonnectedParams, SubscribeParams, WsActionType, WsAuthenticatedParams, WsConnectParams } from "@/models/ws-action-types";
...
export const establishWsConn = ({ id, reconn = false, url }: WsConnectParams): WsActionType<WsConnectParams> => {
...
// @temporary hide
export const requestConnectToAdminRisk = (): WsActionType<WsConnectParams> => establishWsConn({
...
export const sendSubscribe = ({ params, id, requestId }: SubscribeParams): WsActionType<SubscribeParams> => ({
  type: WS_REQUEST_SUBSCRIBE,
  id,
  payload: { params, requestId }
});

export const sendUnsubscribe = ({ params, id, requestId }: SubscribeParams): WsActionType<SubscribeParams> => ({
  type: WS_REQUEST_UNSUBSCRIBE,
  payload: { params, requestId },
  id
});

export const requestAuthWs = ({ data, id }: WsAuthenticatedParams): WsActionType<WsAuthenticatedParams> => ({
...
//@todo
export const requestUnauthWs = ({ data, id }: WsAuthenticatedParams): WsActionType<WsAuthenticatedParams> => ({
  type: WS_UNAUTH,
  payload: { data },
  id,
})

export const wsDisconnected = ({ id, errorCode }: DisonnectedParams): WsActionType<DisonnectedParams> => ({
