...
const DELAY = 2000;
export function createFakeNOOPLoginStream(
  username: string,
  ...
  if (isFailed) {
    ...
    return throwError(errorResponse).pipe(delay(DELAY));
  }
  ...
  return defer(() => of(demoResponse).pipe(delay(DELAY)));
}

export function createFakeACTIONLoginStream(
  username: string,
  isFailed: boolean = false
): Observable<any> {
  if (isFailed) {
  ...
  return of(demoResponse).pipe(delay(DELAY));
}
