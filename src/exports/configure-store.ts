...
export function configureStore(reducers, epics = {}, ) {
  ...
  const rootEpic = (action$, store$, dependencies) => epic$.pipe(
  ...
  SingletonWSManager.getEntries$().subscribe((ids) => {
    ...
    if(streams.length) {
      ...
      const wsEpic = (action$, state$) => merge(
        ...streams.map(s => s(action$, state$))
      ).pipe(
        takeUntil(SingletonWSManager.getEntries$().pipe(
