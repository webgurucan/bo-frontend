...
export function makeRequest<T>(
...
): Observable<T | MakeRequestError | any> {
  options = { ...defaultOption, ...options };

  const url = new URL(
    `${
      options.tradeType === AppTradeType.SPOT ? config.http : config.fhttp
    }/${endpoint}`
  );
  ...
  if (retry) {
    ...
    fetch$ = fetch$.pipe(
      retryWhen((errors) => errors.pipe(delay(delayInMs), take(attempts))),
