...
export type ObservableWorkerConstructor<I = any, O = any> = new (
  ...args
) => DoableWorker<I, O>;
...
export function getWorkerResult<I, O>(
  ...
  return workerIsUnitType(worker)
    ? input$.pipe(
        mergeMap((input) => worker.workUnit(input).pipe(materialize()))
      )
    : worker.work(input$).pipe(materialize());
}

export function runWorker<I, O>(
  workerConstructor: ObservableWorkerConstructor<I, O>
): Subscription {
  ...
  return getWorkerResult(worker, incomingMessages$).subscribe(
    (notification: Notification<O>) => {
      ...
      const workerPostMessage =
        postMessage as unknown as WorkerPostMessageNotification<O>;

      if (transferableWorker && notification.hasValue) {
        workerPostMessage(
          notification,
          worker.selectTransferables(notification.value)
        );
      } else {
