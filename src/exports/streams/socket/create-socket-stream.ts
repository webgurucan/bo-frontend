import { switchMap, mergeMap, filter, catchError, map, tap, ignoreElements, withLatestFrom, bufferTime, take, delay } from "rxjs/operators";
import { throwError, EMPTY } from "rxjs";
import { ActionsObservable, ofType, StateObservable } from "redux-observable";
import { WS_CONNECT } from "@/actions/ws.actions";
import { StreamingWS } from "./socket.stream.class";
import { WebSocketKindEnum, WebSocketKindStateEnum } from "@/constants/websocket.enums";
...
export const initWSStream = (wsId: WebSocketKindEnum, {
  ...
  filter(action => action.id === wsId && !SingletonWSManager.hasInstance(wsId)),
  map((action: WsActionType<WsConnectParams>) => {
    const { id } = action;
    const { url, reconn } = action.payload;
    console.log('[connect] setting ws stream', 'id:', id, 'url:', url, 'reconn', reconn);
    const ws$ = new StreamingWS({ url, id, reconn, subTransformer, unsubTransformer, binaryType: isBinary });
