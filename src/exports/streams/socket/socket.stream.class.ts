...
import { map, switchMap, take, takeUntil, tap, filter, catchError, skipUntil, delay, ignoreElements, withLatestFrom } from "rxjs/operators";
import { of, Subject, merge, race, timer, EMPTY, interval, iif, Observable } from "rxjs";
import { ActionsObservable, ofType, StateObservable } from "redux-observable";
import { WS_OPEN, WS_REQUEST_AUTH, WS_DISCONNECTED, WS_ON_MESSAGE, WS_SEND, closeWs, sendWsData, establishWsConn, WS_REQUEST_CLOSE, WS_REQUEST_SUBSCRIBE, WS_REQUEST_UNSUBSCRIBE, WS_UNAUTH, requestAuthWs, sendSubscribe, openWs, wsDisconnected } from "@/actions/ws.actions";
import { Action } from "@/models/action";
import { DisonnectedParams, SubscribeParams, WsActionType, WsAuthenticatedParams } from '@/models/ws-action-types';
...
export class StreamingWS {
  ...
  public connect(action$, state$: StateObservable<any>) {
    ...
    if (this._binaryType) {
      wsConfig = {
        ...
        serializer: (value: any) => {
          for (var b = new Int8Array(value.length), d = 0; d < value.length; d++) b[d] = value[d];
          return b.buffer
        },
        deserializer: ({ data }) => {
    ...
    const message$ = this._createMessageStream(this.websocketSubject, this._onMessageTransformer)(action$);
    const subscribers$ = this._createSubscribeStream(this._subTransformer)(action$);
    const unsubscribers$ = this._createUnsubscribeStream(this._unsubTransformer)(action$);
    //tmp comment
    ...
    return merge(open$, auth$, beforeClose$, message$, write$, retryConn$, subscribers$, unsubscribers$, close$);
  }

  private _createMessageStream(wsSubject, messageTransformer): ReduxObservableStreamType {
    return (action$) => wsSubject.pipe(
      takeUntil(this._stopStream(action$)),
      map((evt: any) => { // {result, id}
        return of({ type: WS_ON_MESSAGE, payload: messageTransformer(evt), _id: this._id })
      }),
      catchError((error) => {
  ...
  private _createWriteStream(wsSubject): ReduxObservableStreamType {
    return (action$) => action$.ofType(WS_SEND).pipe(
      takeUntil(this._stopStream(action$)),
  ...
  private _createSubscribeStream(subTransformer): ReduxObservableStreamType {
    return (action$) => action$.pipe(
      ...
      map((action: Action<SubscribeParams>) => of({
        type: WS_SEND,
  ...
  private _createUnsubscribeStream(unsubTransformer): ReduxObservableStreamType {
    return (action$) => action$.pipe(
  ...
  private _createRetryConnStream(reconnectInterval: number): ReduxObservableStreamType {
    return action$ => action$.pipe(
      ...
      take(1),
      switchMap((action: WsActionType<DisonnectedParams>) => iif(() => {
        const errorCode = action.payload && action.payload.errorCode;
        // removed from manager
        SingletonWSManager.removeInstance(this._id);

        console.warn('needReconn && errorCode !== 1000', this._reconn, errorCode, this._id);
        return this._reconn && errorCode !== 1000;
      }, of(establishWsConn({ id: this._id, reconn: this._reconn, url: this._url })).pipe(
        tap(() => console.log(`[ws] retry connection after ${reconnectInterval}s`)),
  ...
  private _createHeartBeatStream(pingTimeout: number, pongTimeout: number): ReduxObservableStreamType {
    return (action$) => {
      ...
      const ping$ = interval(pingTimeout).pipe(
        skipUntil(
          action$.ofType(WS_OPEN).pipe(filter(this._wsIdentify))
        ),
        takeUntil(this._stopStream(action$)),
        map(() => of(this._sendData({ event: 'ping' }))),
        tap(() => console.log(`[ws ${this._id}] ping sent after ${pingTimeout}s`))
      );

      const terminate$ = timer(pongTimeout).pipe(
        takeUntil(this._stopStream(action$)),
        map(() => of(closeWs({ id: this._id, reconn: this._reconn }))),
        tap(() => console.log(`[ws ${this._id}] disposed after ${pongTimeout}s`))
      );

      const isPongMessage$ = action$.pipe(
        ofType(WS_ON_MESSAGE),
        filter((action: any) => action.payload && action.payload.event === 'pong'),
      ...
      const pongOrTerminate$ = action$.pipe(
        ofType(WS_SEND),
        filter((action: any) => action.payload && action.payload.event === 'ping'),
  ...
  // auth stream
  private _createAuthStream(action$: ActionsObservable<any>, state$: StateObservable<any>) {
    return action$.pipe(
      ...
      takeUntil(this._stopStream(action$)),
      withLatestFrom(state$),
      filter(([, state]) => wsCollectionSelector(state)[this._id] !== WebSocketKindStateEnum.AUTHORIZED),
      map(([action]: [WsActionType<WsAuthenticatedParams>, any]) => action.payload.data),
      map((data) => {
  ...
  // unath stream
  private _createUnauthStream(action$: ActionsObservable<any>, state$: StateObservable<any>) {
    return action$.pipe(
      ofType(WS_UNAUTH),
      filter(this._wsIdentify),
      takeUntil(this._stopStream(action$)),
