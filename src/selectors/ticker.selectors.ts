...
export const getTickerObj = createSelector<any, TickerModel[], TickerObject>(
	getTickers,
	(items) => items.reduce((result, ticker: TickerModel) => {
		const { symbol } = ticker;
		return {
			...result,
			[symbol]: ticker
 ...
 */
export const getTickerBySymbol = createSelector<any, TickerObject, (string) => TickerModel>(
	getTickerObj,
	(tickers) => _memoize((symbol: string) => tickers[symbol])
);
