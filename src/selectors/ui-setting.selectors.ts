...
export const getSetting = createSelector(
  getSettingReducer,
  (settings) => _memoize((key: string) => settings[key])
);
...
export function getTableSorting(state, { name, defaultSortBy, defaultSortDirection }) {
  ...
  return {
    ...
    defaultSortDirection: sortDirection === 'ASC' || sortDirection === 'DESC' ? sortDirection : null
  };
}
...
export function isFavorite(state, symbol) {
  ...
  return !!(~favorites.map(symbol => symbol.toUpperCase()).indexOf(symbol.toUpperCase()))
}