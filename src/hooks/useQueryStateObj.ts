...
function useHashQueryStringInterface({
	...
	const hasQSI = useMemo(() => ({
		...
		setQueryString(newQueryString: string, { method = 'replace' }) {
			...
			window.history[method === 'replace' ? 'replaceState' : 'pushState'](window.history.state, '', `${history.location.pathname}#${newQueryString}`);
	...
	useEffect(() => {
		...
		window.addEventListener('hashChange', hashChangeHandler, {capture: false});

		return () => window.removeEventListener('hashchange', hashChangeHandler, {capture: false});
	}, [enabled]);
 ...
 * }} queryStateOpts 
 */
export function useQueryStateObj(defaultQueryState: Object, queryStateOpts: QueryStateOptions = {}) {
