...
import { QueryStateOptions, useQueryStateObj, useReactRouterQueryStringInterface } from './useQueryStateObj';
import { parseQueryStateValue, shallowCompareObjects, toQueryStateValue } from '@/exports';
 ...
 * }
 */
export function useQueryString(name: string, defaultValue: any, queryStateOpts?: QueryStateOptions): [any, Function] {
	...
	const setQueryStateItem = useCallback((newValue, opts) => {
		...
		if (
			...
			!(newQueryStateValue !== null && typeof parseQueryStateValue(newQueryStateValue, defaultValue) === typeof defaultValue)
		) {
			console.warn(`value of ${JSON.stringify(newValue)} is not supported ${name} will reset to default value`, defaultValue);
		...
		if (
			(Array.isArray(defaultValue) && isSameJsonString(newValue, defaultValue)) ||
	...
	if (queryStateValue !== undefined || typeof queryStateValue === typeof defaultValue) {
...
export function useQueryStrings(defaultQueryState, queryStateOpts?: QueryStateOptions): [any, Function] {
	// default value is not allowed to be changed after init
	...
	// (obj, obj) => void
	const setQueryStateItem = useCallback((newValues, opts) => {
