...
export const Tabs = React.memo(({
  ...
  tabClassName,
    ...
    <div className={classNames('tabs', getTabClassesByType(tabType), containerClassName, {
      'tabs--disabled': disabled
    })} style={containerStyle}>
      {elements.map(elm => {
        const { active, onClick, title = '', to = '', style = {}, dropdownOptions, className: tabChildClassName } = elm;
        ...
        if (isDropdown) {
          let found = dropdownOptions.find(({ value }) => _isEqual(value, selected));
          ...
          item = <SelectDropdown
            ...
            //@ts-ignore
            onChange={onClick || function ({ value, label }) {
              onChange(value);
            }}
          />;
        }
        ...
        if (!isDropdown) {
          //@ts-ignore
          props.onClick = onClick || function (e) {
            e.preventDefault();
            e.stopPropagation();
            onChange(to);
          };
