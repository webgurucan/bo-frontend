...
type ToastElm = ToastItem & { onRemove: (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => void};
...
class ToastContainer extends Component<any, ToastContainerState> {
	...
	_renderContainer() {
		...
		return (
			<div className={`fixed-container toasts-container ${this.props.position} ${this.props.className || ''}`}>
				<div className="toasts-wrapper">
					<div className="toasts">
						{
							(this.state.isExpanded ? this.state.toasts : this.state.toasts.slice(0, NUMBER_OF_SHOWING_TOAST)).map(toast => {
								let icon = '';
								switch (toast.status) {
									case 'success':
										icon = 'check-circle'
								...
								const toastClasses = classNames('toast', `toast-${toast.status}`, toast.classNames, {
									'toast-animing': toast.runAnim
								});

								return (
									<div key={toast.id} className={toastClasses} onClick={toast.onRemove}>
										<div className="toast__header">
											<div className="toast__icon">
												<Icon id={icon}/>
											</div>
											<div className="toast__header__content">
												{toast.header}
											</div>
										</div>
					...
					</div>
					{
						showMoreBtn ? (
							this.state.isExpanded ?
								<button className="toast__btn" onClick={this.onRemoveAllBtnClick}> Clear All </button> :
								<button className="toast__btn" onClick={this.onClickExpanded}> Show {remainToast} more</button>
						) : null
					}
				</div>
