...
type Toast = {
	[x in ToastType]?: (message: string, header?: string, duration?: number) => void;
} & {
	open: ({ type, message, header, duration }: { type: ToastType; message: string; header?: string, duration?: number; }) => void;
};
