...
export class SelectDropdown extends React.PureComponent<Partial<SelectDropdownProps>, SelectDropdownState> {
  static defaultProps = {
  ...
  parseValue(value, options) {
    ...
    if (this.isValidValue(value)) {
      for (var i = 0, num = options.length; i < num; i++) {
        ...
        } else if (!_isUndefined(options[i].value) && options[i].value === value) {
          option = options[i]
        }
      }
    }
  ...
  renderOption(option) {
    ...
    return (
      <div
        ...
        onClick={!!option.disabled ? null : this.setValue.bind(this, value, label)}
        role='option'
  ...
  buildMenu() {
    ...
    let ops = options.map((option) => {
      if (option.type === 'group') {
        ...
        return (
          <div className={`${baseClassName}-group`} key={option.name} role='listbox'>
            {groupTitle}
            {_options}
          </div>
        )
  ...
  isValueSelected() {
    return this.isValidValue(this.state.selected) || this.state.selected.value !== ''
  }
  ...
  render() {
    ...
    const placeHolderValue = this.isValidValue(this.state.selected) ? this.state.selected : this.state.selected.label
    ...
    return (
      <div className={dropdownClass}>
        <div className={controlClass} onClick={this.handleMouseDown} aria-haspopup='listbox'>
          {value}
