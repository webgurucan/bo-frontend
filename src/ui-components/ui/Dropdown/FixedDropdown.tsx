...
export const FixedDropdown = React.memo(({
  ...
  const toggleContent = useCallback((e) => {
    ...
    const result = getPosition(e, e.target, contentRef.current, place, place, alignContent);
  ...
  const contentClsses = classNames('cpn-dropdown__content', 'hiddencontent', contentClasses);
  const arrowClsses = !!arrowClass ? arrowClass : 'icon-dropdown_arrow_down';

  const _title = _isString(title) ? <span className="text--white">{title}</span> : title;
  const titleCls = `cpn-dropdown__button ${titleClasses}`;

  const content = ReactDOM.createPortal(<div className={secondDropdownClass}>
    <div style={{ ...stylePos, padding: 0 }} className={contentClsses} ref={contentRef}>
      {contentHeader && <div className="cpn-dropdown__header mb-10">{contentHeader}</div>}
  ...
  return (
    <div className='cpn-dropdown'>
      <div className='cpn-dropdown__wrapper'>
        <label className={titleCls} onClick={toggleContent} onMouseDown={stopPropagation}>
          {_title}
          {displayArrow && <span className="cpn-dropdown__arrow">
