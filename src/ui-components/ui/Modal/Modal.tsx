...
import { getResizer, initTopPos, initLeftPos, calculateModalWidth } from './Modal.helpers';
...
class Modal extends React.PureComponent<Partial<ModalProps>, Partial<ModalState>> {
	static defaultProps = {
		isOpen: true,
		initWidth: 200,
		initHeight: 240,
	...
	state = {
		isDragging: false,
		isResizing: false,
		top: this.props.centerTop ? initTopPos({ top: this.props.top, initHeight: this.props.initHeight }) : 50,
		left: initLeftPos({ left: this.props.left, initWidth: this.props.initWidth }),
	...
	onCloseModal(e) {
		const { isOpen, onRequestClose, closeModal, closeAllModals, id, hideAllModalsWhenClose = false } = this.props;
		...
		return (
			<div className={classNames('ui-modal__ctn', this.props.className)}>
				...
				>
					...
					{headerTools && <div className="ui-modal-header-tools">{headerTools}</div>}
					{!disableClose && <span className="close-btn" onClick={this.onCloseModal}>×</span>}
					<div className="ui-modal-body">
						{this.props.children}
						{!disableResize && getResizer({ updateStateResizing: this.updateStateResizing })}
					</div>
				</MultiPurposeModal>
			</div>
		);
