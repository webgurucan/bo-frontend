...
class ConfirmModal extends React.PureComponent<Partial<ConfirmModalDialog>> {
  ...
  renderButtons({ onCancelBtnClick = this.onCancelBtnClick, onOKBtnClick = this.onOKBtnClick } = {}): ReactNode {
    const { cancelText, okText } = this.props;
    return <div className="btn-groups mb-10">
  ...
  render() {
    const { mId, centerTop, title, initWidth, footer, useLegacyBtns, onAccept, onCancel } = this.props;
    ...
    return (
      <Modal
        ...
        headerContent={title ? <div className="text-align-center">{title}</div> : null}
        centerTop={centerTop}
        disableMove={true}
      >
        <div className="confirm-dialog-body">
          {this.props.children({ renderButtons: this.renderButtons, onAccept, onCancel })}
