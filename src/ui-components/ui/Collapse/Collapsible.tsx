...
class Collapsible extends Component<Partial<CollapsibleProps>, CollapsibleState> {
	static defaultProps = {
		transitionTime: 100,
	...
	render() {
		...
		const headerClasses = classNames(`${this.props.classParentString}__header__container`, {
			'collapsible__header__container--closed': this.state.isClosed
		});
		const triggerClasses = classNames(`${this.props.classParentString}__trigger`, openClass, disabledClass, this.state.isClosed ? this.props.titleClassName : this.props.triggerOpenedClassName);
		const parentClasses = classNames(this.props.classParentString, this.state.isClosed ? this.props.className : this.props.openedClassName);
		const outerClasses = classNames(`${this.props.classParentString}__contentOuter`, this.props.contentOuterClassName);
		const innerClasses = classNames(`${this.props.classParentString}__contentInner`, this.props.contentInnerClassName);

		const arrow = !this.props.triggerDisabled && <div className="arrow__container" onClick={this.handleTriggerClick}>
			<Icon id={this.state.isClosed ? 'chevron-right' : 'chevron-down'} />
		</div>;

		const cornerHandler = this.props.overlay && this.props.closeOverlay ? <div className="arrow__container" onClick={this.onCloseOverlay}>
			<Icon id="times" />
		</div> : arrow;
		...
		return (
			<div className={parentClasses}>
				<div className={headerClasses}>
					<div className={`${this.props.classParentString}__title`} onClick={this.handleTriggerClick}>
						{
							//@ts-ignore
							<TriggerElement
							...
							>
								<div className={`${this.props.classParentString}__sub__title__container ${this.props.subTitleClassName}`}>
									{trigger}
									<span className={`${this.props.classParentString}__sub__title`}>{this.props.subTitle}</span>
								</div>
							</TriggerElement>
						}
					</div>
					...
					<div className="collapsible__toolbar">
						{!this.state.isClosed && !this.props.overlay && this.props.toolbar && <div className="d-flex d-align-items-center">
							{this.props.toolbar}
						</div>}
						{cornerHandler}
					</div>
				...
				>
					...
					>
						{children}
						{this.props.overlay && <CollapsibleOverlay classParentString={this.props.classParentString}>
							{this.props.overlay}
						</CollapsibleOverlay>}
					</div>

					{this.props.footer && <div className={`${this.props.classParentString}__footer__container`}>
						{this.props.footer}
					</div>}
				</div>
			</div>
...
export const CollapsibleOverlay = ({ classParentString = 'collapsible', children }: Partial<{ classParentString: string, children?: ReactNode }>) => <div className={`${classParentString}__overlay`}>
	{children}
</div>
