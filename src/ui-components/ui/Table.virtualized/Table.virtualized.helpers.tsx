...
export function getSortedData(args) {
  let { data, sortBy, sortDirection, columns, undefinedOrderSortingFunction } =
    args;
...
export function colNeedsWrap({
  ...
  if (!wrapAtWidth) return 0;

  if (
    tableWidth <= wrapAtWidth &&
    (!wrapLengthLimit || field.length >= wrapLengthLimit)
  ) {
    return 1;
  }
...
export function getNextSort({
  ...
  const next = supportsCustomSort
    ? nextSortOrderTristate[prevSortDirection]
    : nextSortOrder[prevSortDirection];
    ...
    return (
      <div
        ...
        className={classNames(
          "themed-border cpn-virtualized-table__headerrow",
          className,
          headerClasses
        )}
        style={styleObj}
...
export function getHeightForRow({ data, cols, rowHeight, width }, index) {
  ...
  for (let i = 0; i < wrapCols.length; i++) {
    ...
    if (
      width <= wrapAtWidth &&
      (!wrapLengthLimit || fieldData.length >= wrapLengthLimit)
    ) {
      return rowHeight * 2;
    }
  }
...
export function getWrappedColumns(cols) {
  return cols.filter((col) => {
    return (
      (col.wrapFunc || col.dataKey) && (col.wrapLengthLimit || col.wrapAtWidth)
    );
  });
}
...
export function getTableWidthFunc(
  originalWidth,
  enabledHorizontalScroll,
  columns
) {
  if (!enabledHorizontalScroll) return originalWidth;
...
export function renderColumns({
  ...
  const filteredColumns = columns.filter(
    (col) => !col.minWidthForRender || !width || width >= col.minWidthForRender
  );
...
export function getColWidth(columns, index) {
  ...
  //@ts-ignore
  const substr = size.substring(0, size.length - 2);
