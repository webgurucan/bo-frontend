...
export class ScrollBar extends PureComponent<ScrollBarProps> {
		...
		Object.keys(handlerNameByEvent).forEach((key) => {
			...
			if (callback !== prevCallback) {
				if (prevCallback) {
					...
					this._container.removeEventListener(key, prevHandler, {capture: false});
	...
	_updateClassName() {
		...
		if (this._container) {
			this._container.className = `cpn-scrollbar-container${className ? ` ${className}` : ''}${psClassNames ? ` ${psClassNames}` : ''}`;
		}
	}
