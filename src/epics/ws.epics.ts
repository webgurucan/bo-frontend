import { switchMap, filter, map, tap, ignoreElements, take } from "rxjs/operators";
import { EMPTY, of } from "rxjs";
import { ActionsObservable, ofType } from "redux-observable";
import { wsAuthenticated, WS_ON_MESSAGE } from "@/actions/ws.actions";
import { PacketHeaderMessageType, WebSocketKindEnum } from "@/constants/websocket.enums";
...
import { wsOnMessageObservable$, wsOnMessageSrc$ } from "@/exports/streams/socket/on-message.subject";
...
import { InstrumentManner, InstrumentResponseType } from "@/packets/instrument.packet";
import { initInstrument } from "@/exports/ticker.utils";

export const wsOnAdminRiskMessageEpic = (action$: ActionsObservable<any>) => action$.pipe(
  ...
  switchMap((action) => {
    ...
    switch (msgType) {
      case PacketHeaderMessageType.CLIENT_LOGIN: {
        ...
        console.log('ClientLoginManner.read(data);', ClientLoginManner.read(data));

        SingletonWSManager.addWs(`ws://${serverInfo.orderEntryIp1}`, WebSocketKindEnum.ORDERS);
  ...
  switchMap((action) => {
    ...
    switch (msgType) {
      ...
      case PacketHeaderMessageType.TRANSACTION: {
        ...
        console.warn('[TRANSACTION]', orderRaw, 'ordermessageType', orderMessageType);
      ...
      case PacketHeaderMessageType.INSTRUMENT: {
        ...
        return of(updateInstrument(instrument, responseType === InstrumentResponseType.SNAPSHOT_FINISH));
      }
      default: {
        return EMPTY
      }
...
export const wsOnMarketMessageEpic = (action$: ActionsObservable<any>, state$) => action$.pipe(
  ofType(WS_ON_MESSAGE),
...
export const onWebWorkerEpic = (action$, state$) => worker$.pipe(
  // map((batch) => batch[0]),
  switchMap((data: any) => {
    const {
      marketData,
...
export const dataFeed = initWSStream(WebSocketKindEnum.MARKET, { isBinary: false });
