...
export const initBookEpic = (action$: ActionsObservable<any>, state$: StateObservable<any>) => action$.pipe(
  ofType(BOOK_INIT),
  map(action => action.payload),
  distinctUntilChanged((prevPayload, payload) => payload.symbol === prevPayload.symbol && payload.limit === prevPayload.limit),
  ...
  map((data) => {
    ...
    return bookInitialized({ asks: convertToBookData(asks), bids: convertToBookData(bids), lastUpdateId })
  })
);