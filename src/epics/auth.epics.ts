import { logoutSuccessAction, USER_AUTHORIZED, USER_LOGIN_2FA, USER_LOGIN_SUCCESS, USER_LOGOUT_SUCCESS, USER_REQUEST_LOGOUT } from "@/actions/auth.actions";
import { ofType, StateObservable } from "redux-observable";
import {  map, tap, mapTo, mergeMap, withLatestFrom, filter, switchMap, catchError } from "rxjs/operators";
import { LoginType } from "@/fakers/auth.faker";
import { deleteFromStorage, navigate, saveToStorage } from "@/actions/app.actions";
...
// redirect to /
export const loginSuccessEpic = (action$, state$: StateObservable<any>) => action$.pipe(
...
// redirect to 2fa
export const login2FAEpic = (action$) => action$.pipe(
  ofType(USER_LOGIN_2FA),
  mapTo(navigate(RoutePaths.TWO_FACTOR))
);
...
export const logoutEpic = (action$, state$) => action$.pipe(
  ...
  switchMap(([action]) => 
    defer(() => from(fetch('https://jsonplaceholder.typicode.com/todos/1'))).pipe(
